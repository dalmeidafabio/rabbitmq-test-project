package com.f4biu.rabbitmq.common;
...
public class EventStreamConfig {
    ...
    RabbitListenerContainerFactory<StreamListenerContainer> streamContainerFactory(Environment env) {
        ...
        factory.setConsumerCustomizer(
                (id, builder) ->
                        //builder.name("testeZero")
                        builder.name(applicationName)
                                .offset(OffsetSpecification.first())

                                .manualTrackingStrategy()
                                //.autoTrackingStrategy().flushInterval(Duration.ofHours(1))
                                //.autoTrackingStrategy().messageCountBeforeStorage(10)
